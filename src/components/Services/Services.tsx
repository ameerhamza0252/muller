...
export default async function ServicesList({blok}:{blok:any}){
  ...
  //console.log(blok)
    return(
        <div className=" flex flex-col h-auto py-[30px] lg:py-[95px] px-[10px] md:px-[24px] xl:px-[34px] " style={{backgroundColor:blok.background_color,color:blok.text_color}} id={blok.anchor_id} {...storyblokEditable(blok)}>
          ...
          <div className=' grid grid-cols-1 md:grid-cols-2 justify-center gap-[20px] lg:px-[10px] mt-[30px] lg:mt-[65px]'>
            <ItemsCard divider_color={blok.card_divider_color} link_variant={blok.link_variant} buttontext={blok.buttontext} blok={blok.services} />
          </div>
        </div>
    )
}
